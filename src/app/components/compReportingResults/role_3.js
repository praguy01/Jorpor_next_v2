  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const AddData = {user_IdValue , fetch_role_3: true};
...
const generatePDF = async () => {
  ...
  // container.style.border = "1px solid #000"; // เพิ่มเส้นขอบสำหรับการทดสอบ
  // container.style.marginLeft = "20px"; // Add left margin

  const content = document.createElement("div");
  content.id = "pdf-content";
  content.innerHTML = `
      <div class="text-center mb-4"> 
          <h1 class=" text-black  ">แบบรายงานผลการตรวจสอบความปลอดภัย</h1> <!-- Tailwind classes for text size, color, and margin-top -->
      </div>
    ...
    nameExamine.items.forEach((item, index) => {
      ...
      if (item.examine) {
        Object.keys(item.examine).forEach((examKey, examIndex) => {
          ...
          console.log("4444: ", examKey);
          console.log("5555: ", item.examine[examKey][0].itemA);
          console.log("8888: ", item.examine[examKey][0].useEmployee);
  
          if (item.examine[examKey][0].useEmployee === 'false') {
            const examineHeader = document.createElement("h2");
            examineHeader.classList.add("text-left", "text-black", "text-[20px]", 'font-bold', "ml-[10px]", "mt-[10px]");
            ...
            if (item.examine && item.examine[examKey][0].itemA) {
              ...
              // Create and append the header cells
              ["No", "Name", "Status", "Details"].forEach((headerText) => {
                const th = document.createElement("th");
                th.scope = "col";
                th.style.whiteSpace = 'nowrap';
                th.className = "border w-[200px] py-1 text-[12px] text-gray-500 uppercase tracking-wider";
...
//     // Open a new browser window to display the PDF
...
{/*                     
 ...
 export default  CompReportingResults;
