...
function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      const AddData = { storedId ,data_role_1:true};
  ...
  const addActivity = async () => {
    ...
    if (newStartTime !== '' && newEndTime !== '' && newActivity !== '' && selectedDate !== null) {
      ...
      const AddData = {useMeetingAsString ,newStartTime, newEndTime, newActivity ,formattedDate ,storedUser_id, add_role_1: true };
      const data = JSON.stringify(AddData);
  ...
  const deleteTodo = async (item) => {
    try {
      ...
      const editedData = { item , id, edit_role_1: true };
              ...
              <table className='tracking-wider mx-auto px-4 w-[950px]  text-center table-auto shadow '>
                ...
                <tbody className=''> 
                  ...
                  {schedule.map((item,index) => (
                     <tr
                     key={item.id}
                     className={`h-[55px] text-black text-[15px] ${
