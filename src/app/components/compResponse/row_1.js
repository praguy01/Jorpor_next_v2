    ...
    <div className=' bg-[url("/bg1.png")] overflow-auto bg-cover bg-no-repeat absolute z-[-1] top-0 left-0 w-full h-full bg-center   '>
            <div className='md:w-[1000px] mx-auto '>
                ...
                {todoList.map((todo, index) => (
                  <Link href={`/responsedetail_row_1?response=${todo.title}&id=${todo.id}`} key={index}>
                  <div key={index} className={'mx-auto mt-[8px] w-[250px] p-2 h-[100px] md:h-[95px] md:w-[700px] px-2 text-black flex-col bg-[#FFF] text-center rounded-[15px] '}>
                    ...
                    <div className='items-center mt-[5px] text-[11px] md:text-[14px] md:h-[30px] h-[25px] border-[#64CE3F] bg-[#64CE3F] px-5 md:px-10 rounded-[20px]  text-[#fff] flex'>{todo.Verification_status}</div>
